...
export function ScrumboardLayout() {
  ...
  const { isSidebarOpen, setIsSidebarOpen } = useScrumboardAppContext();
  const { data: boards = [], isLoading } = useGetScrumboardBoards();
  const { mutate: createBoard } = useCreateScrumboardBoard();
    ...
    createBoard(
      ...
      },
      {
        onSuccess: (board) => {
          ...
          navigate(`/scrumboard/boards/${board._id}`);
        },
      }
    );
  };
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      >
        ...
        <div className="p-4 border-b border-gray-800">
          <h1 className="text-xl font-bold">Scrumboard</h1>
        </div>
          ...
          <div className="space-y-2">
            {isLoading ? (
              ...
              boards.map((board) => (
                <button
                  key={board._id}
                  onClick={() => navigate(`/scrumboard/boards/${board._id}`)}
