...
interface UseMemberReturn {
  ...
  inviteMember: (data: { email: string; role: "ADMIN" | "TEAM_LEAD" | "PROJECT_MANAGER" | "MEMBER" | "VIEWER" | "WATCHER"; workspaceId: string }) => Promise<Member | null>;
...
export const useMember = (): UseMemberReturn => {
  ...
  const inviteMemberCall = useCallback(
    async (data: { email: string; role: "ADMIN" | "TEAM_LEAD" | "PROJECT_MANAGER" | "MEMBER" | "VIEWER" | "WATCHER"; workspaceId: string }): Promise<Member | null> => {
      setLoading(true);
      setError(null);
      try {
        if (!data.workspaceId) {
